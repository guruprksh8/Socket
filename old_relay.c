...
void rcv_from_clientLOGS(SNDPKT p, int pd)
{
    ...
    ltime = time(NULL);
    if(pd)
    printf(" RELAY 2 | D | %ld | %d | DATA  | %d | CLIENT | RELAY 2 \n", ltime, HEADER_SIZE+p.pay_size, p.seq_no);
    else
    printf(" RELAY 2 | R | %ld | %d | DATA  | %d | CLIENT | RELAY 2 \n", ltime, HEADER_SIZE+p.pay_size, p.seq_no);
}
...
void send_to_clientLOGS(ACKPKT ap)
{
    time_t ltime; 
    ltime=time(NULL);
    printf(" RELAY 2 | S | %ld | %d | ACK | %d | RELAY 2  | CLIENT \n", ltime,ACKPKT_SIZE, ap.seq_no);
}
...
void rcv_from_serverLOGS(ACKPKT ap)
{
    time_t ltime; 
    ltime = time(NULL);
    printf(" RELAY 2 | R | %ld | %d | ACK  | %d | SERVER | RELAY 2 \n", ltime, ACKPKT_SIZE , ap.seq_no);
}
...
void send_to_serverLOGS(SNDPKT p)
{
    time_t ltime; 
    ltime=time(NULL);
    printf(" RELAY 2 | S | %ld | %d | DATA | %d | RELAY 2 | SERVER \n", ltime,HEADER_SIZE+p.pay_size , p.seq_no);
}
...
int main()
{
    ...
    ConfigureConnection(&thisOne,6001);
